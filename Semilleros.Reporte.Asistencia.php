...
// Agarra los numeros (Esto los consigue a todos)
 ...
 //   var_dump($numbers);
	...
	foreach ($number  as $key =>$valor ) {
 
 Execute ("INSERT INTO `Escuelas_Asistencia` (`registro`,`fecha`) VALUES('$valor','$_GET[fecha]');");
}
...
<?php } ?>
  ...
  <button type="submit" class="btn btn-primary mb-2">Generar hoja Asistencia</button>
...
<tr><td>Modalidad</td>
<td> <?php echo  ExecuteScalar("SELECT `teatro13_temporada`.`sem_modalidades`.`modalidad` FROM `teatro13_temporada`.`sem_modalidades` WHERE `teatro13_temporada`.`sem_modalidades`.`Id` = $filtro"); ?>
</td>
<tr>
<td>MES</td><td></td>
</tr>
...
<table class="table table-bordered" style="text-transform:uppercase">
<tr><td></td>
<td style="width:300px">Nómina de Alumnos</td>
...
SELECT
  ...
  `teatro13_temporada`.`sem_alumnos`.`Id` AS `IdAlumno`
FROM
  ...
  INNER JOIN `teatro13_temporada`.`sem_matriculas` ON
    ...
    `teatro13_temporada`.`sem_matriculas`.`alumno`
WHERE
  `teatro13_temporada`.`sem_matriculas`.`modalidad` = '$filtro'
  AND
...
<?php echo $obj->apellidos;?> <?php echo $obj->nombres;?>
</td>
