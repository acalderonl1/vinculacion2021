...
WHERE
  `teatro13_temporada`.`sem_modalidades`.`Id` = $filtro");
...
<option value="0" <?php if($filtro=='0'){echo 'selected';}?>> Todos las Areas de Semilleros</option>
...
<?php } ?>
  ...
  <!--<a class="btn btn-default  mb-2" href="Semilleros.Reporte.HojaAlumno.php?filtro=<?php echo $filtro;?>" target="popup" onClick="window.open(this.href, this.target, 'width=800,height=1000'); return false;">Imprimir </a>-->
</form></td>
...
/**************************/
   ...
   $sql = "
SELECT
  `teatro13_temporada`.`sem_matriculas`.`Id`,
  `teatro13_temporada`.`sem_matriculas`.`alumno`,
  `teatro13_temporada`.`sem_matriculas`.`modalidad`,
  `teatro13_temporada`.`sem_alumnos`.`cedula`,
  `teatro13_temporada`.`sem_alumnos`.`apellidos`,
  `teatro13_temporada`.`sem_alumnos`.`nombres`,
  `teatro13_temporada`.`sem_alumnos`.`direccion`,
  `teatro13_temporada`.`sem_alumnos`.`email`,
  `teatro13_temporada`.`sem_alumnos`.`whatsapp`,
  `teatro13_temporada`.`sem_alumnos`.`nacimiento`,
  `teatro13_temporada`.`sem_matriculas`.`estado`
FROM
  `teatro13_temporada`.`sem_alumnos`
  INNER JOIN `teatro13_temporada`.`sem_matriculas` ON
    `teatro13_temporada`.`sem_alumnos`.`Id` =
    `teatro13_temporada`.`sem_matriculas`.`alumno`
WHERE
  `teatro13_temporada`.`sem_matriculas`.`modalidad` = '$filtro' AND
  `teatro13_temporada`.`sem_matriculas`.`estado` != '3' 
  ORDER BY 
    `teatro13_temporada`.`sem_alumnos`.`apellidos` ASC
