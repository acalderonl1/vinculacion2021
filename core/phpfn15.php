...
function CurrentProjectID() {
	...
	return "{E325E099-3A39-45D1-AB29-9A0F13A19286}";
}
...
class Breadcrumb
{
	...
	{
		global $Language;
		$this->Links[] = ["home", "HomePage", "../main.php", "ew-home", "", FALSE]; // Home
	}
	...
	public function validateUser(&$usr, &$pwd, $autologin, $provider = "")
	{
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ComparePassword($rs->fields('clave'), $pwd);
					if ($valid) {
						$this->_isLoggedIn = TRUE;
						$_SESSION[SESSION_STATUS] = "login";
						...
						$UserProfile->delete('clave'); // Delete password
