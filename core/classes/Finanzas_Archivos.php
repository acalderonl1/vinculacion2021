...
/**
 * Table class for Finanzas_Archivos
 */
class Finanzas_Archivos extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'Finanzas_Archivos';
		$this->TableName = 'Finanzas_Archivos';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`Finanzas_Archivos`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// Id
		$this->Id = new DbField('Finanzas_Archivos', 'Finanzas_Archivos', 'x_Id', 'Id', '`Id`', '`Id`', 3, -1, FALSE, '`Id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// archivo
		$this->archivo = new DbField('Finanzas_Archivos', 'Finanzas_Archivos', 'x_archivo', 'archivo', '`archivo`', '`archivo`', 200, -1, TRUE, '`archivo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->archivo->Sortable = TRUE; // Allow sort
		$this->fields['archivo'] = &$this->archivo;

		// fecha
		$this->fecha = new DbField('Finanzas_Archivos', 'Finanzas_Archivos', 'x_fecha', 'fecha', '`fecha`', CastDateFieldForLike('`fecha`', 0, "DB"), 133, 0, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		...
		// Saldo
		$this->Saldo = new DbField('Finanzas_Archivos', 'Finanzas_Archivos', 'x_Saldo', 'Saldo', '`Saldo`', '`Saldo`', 4, -1, FALSE, '`Saldo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Estado
		$this->Estado = new DbField('Finanzas_Archivos', 'Finanzas_Archivos', 'x_Estado', 'Estado', '`Estado`', '`Estado`', 3, -1, FALSE, '`Estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`Finanzas_Archivos`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "Finanzas_Archivoslist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "Finanzas_Archivosedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "Finanzas_Archivosadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("Finanzas_Archivosview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("Finanzas_Archivosview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "Finanzas_Archivosadd.php?" . $this->getUrlParm($parm);
		else
			$url = "Finanzas_Archivosadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("Finanzas_Archivosedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("Finanzas_Archivosadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("Finanzas_Archivosdelete.php", $this->getUrlParm());
	}
